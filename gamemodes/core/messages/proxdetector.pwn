...
ProxDetectorEx(playerid, Float:max_range, color, const prefix[], const string[], showid = false, annonated=false, inverted=false, showtagcolor=false, autospacing=true, chat=false) {// Prefix = shit that goes infront of the text. i.e. "** Hades starts his engine." => '** ' is the prefix
	new prox[256];
	if (annonated) {
		...
		// remove "says"
		if (strfind(prox, "говорит:", true) != -1) {
			strdel(prox, 0, 5);
		}
	...
	foreach(new i: Player) {
		...
		SendClientMessage(i, final_color, prox);
	}
	return 1;
}
